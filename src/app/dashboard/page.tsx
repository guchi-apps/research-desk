...
import { OLDEST_WEEK_OFFSET, formatDate, formatDateTime, formatMetrics, formatWeekLabel, getLastCollectedAt, getWeekRange, listIndustryInformation, parseWeekOffset, toStringArray, type BusinessParam, type ImportanceParam, type IndustryInformationListItem, type SourceParam } from "@/lib/industry-information";
...
function Card({ item }: { item: IndustryInformationListItem }) {
  ...
  return <article className={`news-card ${supplement ? "supplement" : ""}`}><div className="news-head"><div><h3>{item.title}</h3><p className="meta">{formatDate(item.publishedAt ?? item.collectedAt)}　·　{item.sourceName}　·　{item.isPrimarySource ? "一次情報" : "関連記事"}</p></div><span className={`badge ${item.importance === "HIGH" ? "high" : ""}`}>{supplement ? "30日以内" : `重要度 ${importanceLabels[item.importance]}`}</span></div><p className="summary">{item.summary ?? "要約は登録されていません。"}</p><dl className="details"><div><dt>主な数値・事実</dt><dd>{formatMetrics(item.extractedMetrics) ?? "—"}</dd></div><div><dt>企画・設計への示唆</dt><dd>{item.implications ?? "—"}</dd></div><div><dt>キーワード</dt><dd>{keywords.length ? keywords.join("／") : "—"}</dd></div></dl><div className="news-foot"><div className="tags">{item.targetCompany && <b>{item.targetCompany}</b>}{item.targetProduct && <span>{item.targetProduct}</span>}{tags.map((tag) => <span key={tag}>{tag}</span>)}</div><a href={item.originalUrl} target="_blank" rel="noreferrer">元記事 ↗</a></div></article>;
}

function Section({ business, items }: { business: Business; items: IndustryInformationListItem[] }) {
...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [visible, lastCollectedAt] = await Promise.all([listIndustryInformation({ weekOffset, business, source, importance, keyword }), getLastCollectedAt()]);
