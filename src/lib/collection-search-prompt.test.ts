...
describe("buildCollectionSearchPrompt", () => {
  ...
  it("不採用記事と調整指示を事業ごとの基準として渡す", () => {
    const prompt = buildCollectionSearchPrompt(NOW, {
      delivery: { policy: "宅配は人事記事を採用しない", instruction: "ポスト社長を避ける", rejectedArticles: ["ポスト社長に就任"], existingArticles: [{ title: "既存の宅配記事", summary: "既存要約", url: "https://example.com/delivery" }] },
      locker: { policy: "ロッカーは芸能記事を採用しない", instruction: "俳優の話題を避ける", rejectedArticles: ["俳優がロッカーを利用"], existingArticles: [] },
    });
